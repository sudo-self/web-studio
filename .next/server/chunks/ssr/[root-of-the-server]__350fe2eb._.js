...
</nav>`,description:"Breadcrumb with icons",tags:["navigation","breadcrumb","icons"]}},Q={Layout:["header","hero","about","services","contact","footer","sidebar"],Navigation:["navbar","breadcrumb","breadcrumb-advanced","mega-menu"],Content:["card","gallery","team","testimonials","stats","timeline","faq"],Forms:["contact","login-form","newsletter","search-bar","toggle-switch"],"UI Components":["modal","progress","pricing","primary-button","secondary-button","button-group"],Documentation:["readme-basic","readme-advanced"],SEO:["seo","seo-schema"],Icons:["social-icons","feature-icons","font-icons"]};function R({onInsert:a,onAiInsert:d,onOpenSettings:e,onResizeStart:f,currentCode:g="",framework:j}){let{settings:l}=(0,O.useSettings)(),[n,o]=(0,c.useState)(""),[p,q]=(0,c.useState)(!1),[r,s]=(0,c.useState)(""),[t,R]=(0,c.useState)("response"),[S,T]=(0,c.useState)([]),[U,V]=(0,c.useState)(""),[W,X]=(0,c.useState)(new Set),[Y,Z]=(0,c.useState)(null),[$,_]=(0,c.useState)([]),[aa,ab]=(0,c.useState)(!1),[ac,ad]=(0,c.useState)(!1),[ae,af]=(0,c.useState)({name:"web-studio-project",description:"Project created with AI Web Studio",isPublic:!0,deployPages:!0}),[ag,ah]=(0,c.useState)(null),[ai,aj]=(0,c.useState)(null),[ak,al]=(0,c.useState)(!1);(0,c.useEffect)(()=>{},[]),(0,c.useEffect)(()=>{},[]),(0,c.useEffect)(()=>{},[W]);let am=async a=>{try{let b=await fetch("https://api.github.com/user",{headers:{Authorization:`Bearer ${a}`,Accept:"application/vnd.github.v3+json"}});if(b.ok){let a=await b.json();aj(a)}}catch(a){console.error("Failed to fetch user info:",a)}},an=(0,c.useMemo)(()=>{if(!U)return Q;let a={};return Object.entries(Q).forEach(([b,c])=>{let d=c.filter(a=>{let b=P[a];return a.toLowerCase().includes(U.toLowerCase())||b.description.toLowerCase().includes(U.toLowerCase())||b.tags.some(a=>a.toLowerCase().includes(U.toLowerCase()))});d.length>0&&(a[b]=d)}),a},[U]),ao=async()=>{if(!ag)return void alert("Please connect to GitHub first");al(!0);let a=window.open("","_blank","width=400,height=200");a&&(a.document.write('<p style="font-family:sans-serif; padding: 20px;">Creating repository…</p>'),a.document.close());try{let b={html:g||`<!DOCTYPE html>
<html>
<head>
    <title>${ae.name}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
      ...
      deploy:
        ...
        steps:
          ...
          - name: Deploy to GitHub Pages
            id: deployment
            uses: actions/deploy-pages@v4`}),g})(b,ae.deployPages,ai,ae),d=await fetch("/api/github/create-repo",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:ae.name,description:ae.description,isPublic:ae.isPublic,deployPages:ae.deployPages,files:c,accessToken:ag})}),e=await d.json();if(e.success){let b=`
          ...
          `;a&&(a.document.open(),a.document.write(b),a.document.close()),ad(!1)}else throw Error(e.error||"Failed to create repository")}catch(b){console.error("GitHub repo creation failed:",b),a&&a.close(),alert(`Failed to create repository: ${b.message}`)}finally{al(!1)}},ap=async()=>{if(!n.trim())return void s("Please enter a prompt");if(aa||p)return void console.log("Request already in progress, ignoring...");ab(!0),q(!0),s("");try{console.log("Sending AI request:",{prompt:n.substring(0,100),mode:t,framework:j,timestamp:new Date().toISOString()});let a="react"===j?`You are an expert React developer. Create a React component with JSX and inline styles for: "${n}"
