...
URL: ${d.html_url}
${d.pages_url?`Pages: ${d.pages_url}`:""}`),ac(!1),d.html_url&&window.open(d.html_url,"_blank");else throw Error(d.error||"Failed to create repository")}catch(a){console.error("GitHub repo creation failed:",a),alert(`Failed to create repository: ${a.message}`)}finally{ak(!1)}},ao=async()=>{if(!l.trim())return void r("Please enter a prompt");if(_||o)return void console.log("Request already in progress, ignoring...");aa(!0),p(!0),r("");try{console.log("Sending AI request:",{prompt:l.substring(0,100),mode:s,timestamp:new Date().toISOString()});let a=[{role:"user",content:`You are an expert web developer. Create responsive HTML with inline CSS for: "${l}"
...
//# sourceMappingURL=%5Broot-of-the-server%5D__08984e37._.js.map