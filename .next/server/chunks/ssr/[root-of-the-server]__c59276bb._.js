...
</nav>`,description:"Breadcrumb with icons",tags:["navigation","breadcrumb","icons"]}},Q={Layout:["header","hero","about","services","contact","footer","sidebar"],Navigation:["navbar","breadcrumb","breadcrumb-advanced","mega-menu"],Content:["card","gallery","team","testimonials","stats","timeline","faq"],Forms:["contact","login-form","newsletter","search-bar","toggle-switch"],"UI Components":["modal","progress","pricing","primary-button","secondary-button","button-group"],Documentation:["readme-basic","readme-advanced"],SEO:["seo","seo-schema"],Icons:["social-icons","feature-icons","font-icons"]};function R({onInsert:a,onAiInsert:d,onOpenSettings:e,onResizeStart:f,currentCode:g=""}){let{settings:j}=(0,O.useSettings)(),[l,n]=(0,c.useState)(""),[o,p]=(0,c.useState)(!1),[q,r]=(0,c.useState)(""),[s,t]=(0,c.useState)("response"),[R,S]=(0,c.useState)([]),[T,U]=(0,c.useState)(""),[V,W]=(0,c.useState)(new Set),[X,Y]=(0,c.useState)(null),[Z,$]=(0,c.useState)([]),[_,aa]=(0,c.useState)(!1),[ab,ac]=(0,c.useState)(!1),[ad,ae]=(0,c.useState)({name:"web-studio-project",description:"Project created with AI Web Studio",isPublic:!0,deployPages:!0}),[af,ag]=(0,c.useState)(null),[ah,ai]=(0,c.useState)(null),[aj,ak]=(0,c.useState)(!1);(0,c.useEffect)(()=>{},[]),(0,c.useEffect)(()=>{},[]),(0,c.useEffect)(()=>{},[V]);let al=async a=>{try{let b=await fetch("https://api.github.com/user",{headers:{Authorization:`Bearer ${a}`,Accept:"application/vnd.github.v3+json"}});if(b.ok){let a=await b.json();ai(a)}}catch(a){console.error("Failed to fetch user info:",a)}},am=(0,c.useMemo)(()=>{if(!T)return Q;let a={};return Object.entries(Q).forEach(([b,c])=>{let d=c.filter(a=>{let b=P[a];return a.toLowerCase().includes(T.toLowerCase())||b.description.toLowerCase().includes(T.toLowerCase())||b.tags.some(a=>a.toLowerCase().includes(T.toLowerCase()))});d.length>0&&(a[b]=d)}),a},[T]),an=async()=>{if(!af)return void alert("Please connect to GitHub first");ak(!0);let a=window.open("","_blank","width=400,height=200");a&&(a.document.write('<p style="font-family:sans-serif; padding: 20px;">Creating repository…</p>'),a.document.close());try{let b={html:g||`<!DOCTYPE html>
<html>
<head>
    <title>${ad.name}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
      ...
      deploy:
        ...
        steps:
          ...
          - name: Deploy to GitHub Pages
            id: deployment
            uses: actions/deploy-pages@v4`}),g})(b,ad.deployPages,ah,ad),d=await fetch("/api/github/create-repo",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:ad.name,description:ad.description,isPublic:ad.isPublic,deployPages:ad.deployPages,files:c,accessToken:af})}),e=await d.json();if(e.success){let b=`
          ...
          `;a&&(a.document.open(),a.document.write(b),a.document.close()),ac(!1)}else throw Error(e.error||"Failed to create repository")}catch(b){console.error("GitHub repo creation failed:",b),a&&a.close(),alert(`Failed to create repository: ${b.message}`)}finally{ak(!1)}},ao=async()=>{if(!l.trim())return void r("Please enter a prompt");if(_||o)return void console.log("Request already in progress, ignoring...");aa(!0),p(!0),r("");try{console.log("Sending AI request:",{prompt:l.substring(0,100),mode:s,timestamp:new Date().toISOString()});let a=[{role:"user",content:`You are an expert web developer. Create responsive HTML with inline CSS for: "${l}"
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c59276bb._.js.map